...
Util.profileStart('chunks');
        ...
        $.each(data.chunks, function(chunkNo, chunk) {
          ...
          $.each(chunk.fragments, function(fragmentNo, fragment) {
            ...
            var brat_jiana_type = ['pv', 'pa', 'nbu', 'num', 'nbn', 'nq_juso', 'nq_per', 'nq_loc', 'nq_etc', 'ncp', 'ncn', 'np', 'nn', 'eng', 'mm', 'ma', 'j', 'jp', 'xs', 'xsp', 'xp', 'xsn', 'xsn_nq', 'ef', 'ep', 'etn', 'efa', 'i', 'sp', 'sf', 'sl', 'sr', 'sd', 'se', 'su', 'sy', 'ukall'];
            var brat_plot_type = ['$HOW', '$ADVICE', '$ANGRY', '$APOLOGY', '$BLAME', '$BOAST', '$BUT', '$CHIMIN', '$CLOSE', '$COMPORT', '$COMPLAINT', '$CONCERN', '$CONGRATULATION', '$DISLIKE', '$ENVY', '$FACT', '$FEAR', '$FORGIVE', '$GUESS', '$JOKE', '$JOY', '$LIKE', '$LOVE', '$NONE', '$NOT', '$NPFP', '$NPSP', '$NPTP', '$OPEN', '$ORDER', '$PRAISE', '$PROMISE', '$REASK', '$SAD', '$SHYNESS', '$SICK', '$SUGGEST', '$SURPRISE', '$THANKS', '$THREAT', '$WHAT', '$WHEN', '$WHERE', '$WHICH', '$WHO', '$WHY', '$WILL', '$WISH', '$YN', '$BANKING', '$FAMILY', '$BANK_PRODUCT', '$COST', '$CREDITCARD', '$ORG_BANK', '$FUNCTION', '$CERTIFICATE', '$SERVICE', '$STATUS', '$PASSWORD', '$USETERMS', '$COST', '$MILEAGE', '$MOVIESERVICE', '$PPSERVICE', '$PREMIUMSERVICE', '$BLISS5', '$BLISS7', '$FOOD', '$FACILITY', '$OBJ_LOCATION', '$MOVIETITLE', '$MOVIEETC', '$SYSTEM'];
            var type_text = span.type;
            var fgStyle;
            for(var i=0; i<brat_jiana_type.length; i++) {
    			if(type_text.indexOf(brat_jiana_type[i]) > 0) {
    				fgStyle = 'fill:#769ac3;font-size:10px;';
    			}
    		}
    		for(var i=0; i<brat_plot_type.length; i++) {
    			if(type_text.indexOf(brat_plot_type[i]) > 0) {
    				fgStyle = 'fill:#f0a82b;font-size:12px;';
    			}
    		}
            ...
            var fragmentText = svg.text(fragment.group, x, y - span.floor, data.spanAnnTexts[fragment.glyphedLabelText], { 'style': fgStyle });
        ...
        var webFontConfig = {
          custom: {
            ...
            urls: webFontURLs !== undefined ? webFontURLs : [
              '../../../../ttf/static/fonts/Astloch-Bold.ttf',
              '../../../../ttf/static/fonts/PT_Sans-Caption-Web-Regular.ttf',
              '../../../../ttf/static/fonts/Liberation_Sans-Regular.ttf'
            ],
          },
